...
def get_data(filters):
	...
	if filters.document=="Quotation":
		...
		if filters.sales_person:
			conditions += " AND q.custom_sales_personuser = %(sales_person)s"
		...
		for i in quotation:
			if i.status != "Cancelled":
				row = [i.name,i.party_name,i.transaction_date,i.item_code,i.description,i.qty,i.rate,i.amount,i.custom_sales_personuser]
				data.append(row)
	...
	#Sales Order
		...
		if filters.sales_person:
			conditions += " AND so.custom_sales_personuser = %(sales_person)s"
			params["sales_person"] = filters.sales_person
		...
		for i in sales_order:
			if i.status != "Cancelled":
				amt=i.qty*i.rate
				row = [i.name,i.customer,i.transaction_date,i.item_code,i.description,i.qty,i.rate,amt,i.po_no,i.custom_sales_personuser]
				data.append(row)
	...
	#Sales Invoice
		...
		if filters.sales_person:
			conditions += " AND si.custom_sales_personuser = %(sales_person)s"
			params["sales_person"] = filters.sales_person
		...
		for i in sales_Invoice:
			if i.status != "Cancelled":
				if i.stock_transfer!='Stock Transfer':
					amt=i.qty*i.rate
					row = [i.name,i.customer,i.posting_date,i.item_code,i.description,i.qty,i.rate,amt,i.po_no,i.custom_sales_personuser]
					data.append(row)
	...
	#Delivery Note
		...
		if filters.sales_person:
			conditions += " AND dn.custom_sales_personuser = %(sales_person)s"
			params["sales_person"] = filters.sales_person
