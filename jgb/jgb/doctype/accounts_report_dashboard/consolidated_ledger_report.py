...
@frappe.whitelist()
def create_sales_person(employee, is_sales_person):
    emp_name, user_id = frappe.db.get_value("Employee", employee, ["employee_name", "user_id"])
    sales_user_role = "Sales User"
    enable = cint(is_sales_person)
    if enable:
        sp_name = frappe.db.exists("Sales Person", {"sales_person_name": emp_name,"employee":employee})
        if not sp_name:
            sp= frappe.new_doc('Sales Person')
            sp.sales_person_name = emp_name
            sp.enabled =1
        ...
        if user_id:
            ...
            if sales_user_role not in [r.role for r in user.roles]:
                user.append("roles", {"role": sales_user_role})
                user.save(ignore_permissions=True)

            return _("Sales Person created and role assigned")
        else:
            return _("Sales Person created.")
        ...
        if user_id:
            user = frappe.get_doc("User", user_id)
            user.roles = [r for r in user.roles if r.role != sales_user_role]
            user.save(ignore_permissions=True)

            return _("Sales Person disabled and role removed")
        else:
            return _("Sales Person disabled")
