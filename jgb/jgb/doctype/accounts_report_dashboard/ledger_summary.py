...
def get_data(args):
    ...
    for c in li:
        ...
        for g in gle:
            ...
            row1+=[c,float(fmt_money(g.opening_debit - g.opening_credit).replace(',', ''))]
            ...
            for i in sq:
                ...
                row1+=[float(fmt_money(i.debit).replace(',', '')),float(fmt_money(i.credit).replace(',', '')),float(fmt_money(op_debit- op_credit).replace(',', ''))]
                data.append(row1)
    row2 += ["Total",float(fmt_money(balance_op).replace(',', '')),float(fmt_money(total_op_debit).replace(',', '')),float(fmt_money(total_op_credit).replace(',', '')),float(fmt_money(balance_cl).replace(',', ''))]
