...
def make_xlsx(data, sheet_name=None, wb=None, column_widths=None):
    ...
    header4 = ['Supplier: ' + supplier or 'All']
    ws.append(header4 + [""] * 7)
    ws.append([''])
    ...
    for header in ws.iter_rows(min_row=3, max_row=5, min_col=1, max_col=7):
        for cell in header:
            cell.font = bold_font
            # cell.fill = lightgray_fill
    ...
    ws.merge_cells(start_row=len(data1) + 5, start_column=1, end_row=len(data1) + 5, end_column=4)
    ws.merge_cells(start_row=len(data1) + 6, start_column=1, end_row=len(data1) + 6, end_column=6)
...
def get_data(args):
    ...
    for i in gl_entry:
        ...
        row.append(float(fmt_money(round(i.debit, 2)).replace(',', '')) or 0)
        row.append(float(fmt_money(round(i.credit, 2)).replace(',', '')) or 0)
        row.append(float(fmt_money(round(balance, 2)).replace(',', '')) or 0)
        data.append(row)
